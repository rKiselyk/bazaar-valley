...
function TableRowItems({ items, cart, wishlist, comparisonList }) {
			...
			{items.map((item) => {
				return (
					<div
						key={item.id}
						className={
							"d-flex w-100 bg-white with-border cursor-pointer mb-2 " +
					...
					>
						...
						<div className="ml-auto d-flex flex-column p-3">
							{isInCart(item) && (
								<img
									style={{ height: 25 }}
									className="mb-2"
